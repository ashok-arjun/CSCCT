...
class BaseTrainer(object):
    ...
    def set_dataset_variables(self):
        ...
        if self.args.dataset == 'cifar100':
            ...
            self.trainset = torchvision.datasets.CIFAR100(root=datadir, train=True, download=True, transform=self.transform_train)
            self.testset = torchvision.datasets.CIFAR100(root=datadir, train=False, download=True, transform=self.transform_test)
            self.evalset = torchvision.datasets.CIFAR100(root=datadir, train=False, download=False, transform=self.transform_test)
            self.balancedset = torchvision.datasets.CIFAR100(root=datadir, train=False, download=False, transform=self.transform_train)
            ...
            datadir = os.path.join(self.args.data_dir, 'imagenet_sub')
    ...
    def init_current_phase_dataset(self, iteration, start_iter, last_iter, order, order_list, \
        ...
        X_protoset_cumuls, Y_protoset_cumuls, T_train_cumuls, T_valid_cumuls, T_protoset_cumuls):
        ...
        # Get the task IDs
        T_train = np.full(Y_train.shape, iteration, dtype=int)
        T_valid = np.full(Y_valid.shape, iteration, dtype=int)
        T_valid_cumuls.append(T_valid)
        T_train_cumuls.append(T_train)
        T_valid_cumul = np.concatenate(T_valid_cumuls)
        T_train_cumul = np.concatenate(T_train_cumuls)
        ...
        if iteration == start_iter:
            ...
            T_protoset = np.concatenate(T_protoset_cumuls)
            ...
            T_train = np.concatenate((T_train, T_protoset))
        ...
        if iteration == start_iter:
            return indices_train_10, X_valid_cumul, X_train_cumul, Y_valid_cumul, Y_train_cumul, X_train_cumuls, Y_valid_cumuls, \
                ...
                map_Y_valid_cumul, X_valid_ori, Y_valid_ori, T_train, T_train_cumuls, T_valid, T_valid_cumuls, T_valid_cumul, T_train_cumul
        else:
            return indices_train_10, X_valid_cumul, X_train_cumul, Y_valid_cumul, Y_train_cumul, X_train_cumuls, Y_valid_cumuls, \
                X_protoset_cumuls, Y_protoset_cumuls, X_valid_cumuls, Y_valid_cumuls, X_train, map_Y_train, \
                map_Y_valid_cumul, X_protoset, Y_protoset, T_train, T_train_cumuls, T_valid, T_valid_cumuls, T_valid_cumul, T_train_cumul
    ...
    def set_exemplar_set(self, b1_model, b2_model, is_start_iteration, iteration, last_iter, order, alpha_dr_herding, prototypes):
        ...
        if self.args.dataset == 'cifar100':
            ...
            for iteration2 in range(iteration+1):
                for iter_dico in range(self.args.nb_cl):
                    ...
                    T_protoset_cumuls.append(np.full(Y_protoset_cumuls[-1].shape, iteration2, dtype=int))
        ...
        return X_protoset_cumuls, Y_protoset_cumuls, class_means, alpha_dr_herding, T_protoset_cumuls

    def create_label_to_task_mapping(self, labels, tasks):
        if type(labels) == np.ndarray:
            labels = list(labels)
        
        if type(tasks) == np.ndarray:
            tasks = list(tasks)

        return dict(zip(labels, tasks))
